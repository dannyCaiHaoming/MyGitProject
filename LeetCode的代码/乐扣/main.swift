...
//		var longest:String = ""
    ...
    func longestCommonPrefix2(_ strs: [String]) -> String {
        ...
        while low <= high {
            let mid = (low + high) / 2
            if self.longestCommonPrefix2_Support(strs, mid){
                low = mid + 1
            }else{
                high = mid - 1
            }
    ...
    func isValid(_ s: String) -> Bool {
        ...
        while true  {
            ...
            if !stack2.isEmpty && self.isSymbolCompare(front: String(back!), back: String(stack2.peek()!)){
                stack2.pop()
                continue
            }
            ...
            if !self.isSymbolCompare(front: String(front!), back: String(back!)) {
                stack2.push(back!)
            }else{
                stack1.pop()
            }
...
//        var i = 0 //遍历匹配串
    ...
    func
        ...
        nextArr = self.getNextArray(matchStr: needle)
...
//        }
    ...
    func isSymmetric(_ root: TreeNode?) -> Bool {
        return self.isSymmetric_Mirror(root1: root, root2: root)
    }
    ...
    func maxDepth(_ root: TreeNode?) -> Int {
        ...
        let leftMax = self.maxDepth(root?.left) + 1
        let rightMax = self.maxDepth(root?.right) + 1
...
//                nodeArray.append(node)
    ...
    func buildTree(_ preorder: [Int], _ inorder: [Int]) -> TreeNode? {
        ...
        if leftPreTree.count > 0 {
            treeNode.left = self.buildTree(leftPreTree as! [Int], leftInTree as! [Int])
        }
        
        //计算右子树的pre，in数组
        if rightPreTree.count > 0 {
            treeNode.right = self.buildTree(rightPreTree as! [Int], rightInTree as! [Int])
        }
