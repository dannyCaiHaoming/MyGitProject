...
//        print(lastStoneWeightII([2,7,4,1,8,1]))
        ...
        print(obj.maxProfit3([2,1,2,0,1]))
    }
    ...
    //    }
        ...
        /*
         1.i：窗口左侧，j=L+i-1：窗口右侧
         2.注意遍历是长度在外层，这样子可以由小到大的子串开始动态规划
         */
        for L in 2...length {
            for i in 0..<length {
                let j = L + i - 1
    ...
    /*
     ...
     这道题的简单思想，就是从当前位置前记录出一个最小的值，当前的dp，就是当前价格-dp_min[n-1]。dp_min每次更新，对比之前的最小和当下值。
    ...
    和121题目的区别是，当天如果持有的股票的利润值，这道题需要将利润叠加，因此用的是上一天不持有股票最大的利润扣除当天股票的成本，来计算出成本最低的一天。其实就是到那天为止的最大值，加上一个临时的买入股票，如果后续有更便宜的股票买入，这个值就会被刷新。
     */
    ...
    /*
     ...
     dp[i][j][k]
     i表示第几天，j是交易次数，k是买或卖出货
     基本和121,121题目一样，dp[i][k]为在那天买卖数据的最大值。
     只不过加了个次数的限制，需要在之前买卖值的时候，需要加个次数的维度。
     维护了一个2个次数的维度，第一个次数，那就是跟122题目一样。如果增加了一个次数，
     那么当前的最大值，就要考虑次数维度1次和当前次数的最大值，还有前一天跟当天最大次数维度的比较。
     */
