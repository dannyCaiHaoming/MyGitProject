...
简单记住触发条件。CAlayer有三层，分别是背景，content，框框，只要背景上有被其他东西遮挡且改了框框的内容，就需要另外开辟空间存储及另外处理。
...
7. 图片的解码时机和优化
图片使用由UIImage作为model，UIImageView作为view展示，需要先经过load->decode->render,因此真正发生解码的时机，是图片赋值到UIImageView的image上，且这个UIImageView即将要渲染出来之前。因为这个解码的过程是在主线程，且由于和图片尺寸成正比，因此可以尝试将图片解码放入子线程，使用先将图片的信息绘制到bitmap上，等创建出UIImage再回到主线程进行使用。
8. 图片渲染优化
两中情况：
1.遇到大量需要解码的图片，可以将解码放在子线程，提前解码，降低cpu占用情况。
2.如果遇到大型图片加载，可以采用降采样，还有分片加载。CATileLayer。
9.  多个相同图片重复加载
10. imageName
1.使用imageName加载，会先尝试用缓存中获取图片，如果没有则会从bundle加载且加载进缓存中使用，下次会从缓存中查找。
2.imageWithContentsOfFile，会每次重新查找加载图片，但是不会缓存起来。