...
- 分类只能添加属性（声明`setter`和`getter`方法），并不能添加实例变量。实例变量需要满足以下条件：
	- 声明了一个成员变量(运行时添加方法，但是不能修改实例变量内存中的布局。)
...
- 此外，必须是对象的属性进行观察，即因为kvo需要吊起一个属性的setter方法，才能对里面通知外部的方法进行使用！所以如果使用kvc的话，就不会执行这个通知了。
...
## 4.2 alloc] init] 和 new方法对比
new其实就是调用alloc]init],只不过alloc]init]可以实现自定义的init方法

## 4.3 allocWithZone 与单例
一般单例是用dispatch_once在init方法让全局只返回同一个实例。
而真正的单例实现是在allocWithZone方法内全局返回同一块内存空间。
此外最好把NSCoping的方法也用同一个实例返回。
