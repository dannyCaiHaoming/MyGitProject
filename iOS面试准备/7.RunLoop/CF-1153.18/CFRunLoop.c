...
/*
注解：
1.RunloopMode包含多个source，observer,timer
...
/*
注解：
1.可以看出CFRunLoop是基于pthread管理的
2.不允许直接创建Runloo，只能通过`CFRunLoopGetMain`,`CFRunLoopGetCurrent`获取
*/
...
注解：
1.RunLoop执行的步骤
2.source0是应用层的消息，source1是内核级别的消息，所以是通过mach_port传送的
*/
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode)
{
    ...
    do
    {
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime)
        {
            ...
            //注解：如果有 Source1 (基于port) 处于 ready 状态，直接处理这个 Source1 然后跳转去处理消
        ...
        do
        {
            ...
            msg = (mach_msg_header_t *)msg_buffer;
            /*
            注解： 调用mach_msg接收到mach_port的消息。线程进入休眠直到时间被触发
            - 基于port的source时间
            - timer的时间到
            - runloop自身时间到
            为啥是这几个看下面处理方法
...
// #endif
        ...
        else if (rlm->_timerPort != MACH_PORT_NULL && livePort == rlm->_timerPort)
        {
            ...
            //注解： 如果是timer的时间到了，则触发这个timer的回调
            if (!__CFRunLoopDoTimers(rl, rlm, mach_absolute_time()))
            {
        ...
        {
            //注解：如果有有分发到main_queue的block，执行block
        ...
        */
            ...
            if (rls)
            {
                ...
                //注解： __CFRunLoopServiceMachPort 使得runloop进入睡眠
                // 注解： 从这里可以看到上文__CFRunLoopServiceMachPort（基于port的）消息是传送到source1执行
