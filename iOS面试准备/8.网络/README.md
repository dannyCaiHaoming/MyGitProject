...
- 特点
	...
	- `面向字节流`
	- `滑动窗口控制`<br>
	由于如果每次客户端发送请求到服务器，都需要等待服务器成功返回才能发下一个，那么这个效率会非常的低，滑动窗口控制，就是可以在这个窗口范围内，可以不用等待请求返回可以最多发送多少个报文的数量。这样就可以大大减少等到正确序号响应返回等待的时间。
	- `流量控制`<br>
	用于避免滑动窗口过大，导致服务器缓存区溢出，无法继续处理后续客户端发送的数据，到就要满的时候可以告诉客户端减少窗口大小，降低发送报文数量
	- `拥塞控制`
		- 慢开始，拥塞控制，一开始是指数增长，到达门限值开始线性增长，直到达到网络拥塞
		- 快恢复，快重传
...
- 创建Socket
- 客户端调用`connect`向指定ip跟端口的服务器发起连接
- 客户端与服务器建立连接之后，就可以开始发送消息
- 客户端可以调用`close`关闭socket

#### 8.6.2 心跳机制

- TCP默认的keep-alive监测时间太长
- keep-alive只能监测服务器是否存活，但不能知道服务器这时候是否还能服务（100%cpu使用率的服务器）
...
- 客户端发送`ping`，一段时间内没收到服务器的`ack`，则客户端认为不在线了，主动断开`socket`连接
- 服务器端也一样，主动断开`socket`连接，会改由`APNs`推送消息
