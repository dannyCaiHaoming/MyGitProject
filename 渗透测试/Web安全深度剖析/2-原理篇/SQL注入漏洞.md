...
严格来说，HttpOnly对防御XSS漏洞不起作用，**主要目的是为了解决XSS漏洞后续的Cookie劫持攻击**。
...
- require：找不到包含文件会报错，并停止脚本
- include：找不到包含文件只会警告，脚本继续执行
...
如果page字段为空则去home，如果非空则去找页面包含进来去访问。但是如果尝试访问后台没有的页面，就会报错。把后端的文件目录路径显露出来。
